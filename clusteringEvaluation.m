function result=clusteringEvaluation(groups,classLabels)
    % groups is n*m matrix
    % m is the number of samples
    % n is the number of clustering results
    % classLabels is 1*m vector
    ...
    % return ACC, Purity and NMI as evaluation of clustering
    % input variables are column, return rows of evaluation
    result=zeros(size(groups,1),3);

    for i=1:size(groups,1)
        res = bestMap(classLabels,groups(i,:))';
        % evaluate AC: accuracy
        AC = length(find(classLabels == res ))/length(classLabels);
        % evaluate Purity: purity
        Purity = purity(classLabels, groups(i,:));
        % evaluate NMI: nomalized mutual information
        NMI = MutualInfo(classLabels, groups(i,:) );
        result(i,:)=[AC,Purity,NMI];
    end
end
...
if(nClass1<nClass)
    tmp=-1*ones(nClass,1);
    tmp(1:nClass1)=Label1;
    Label1=tmp;
end
if(nClass2<nClass)    
    tmp=-1*ones(nClass,1);
    tmp(1:nClass2)=Label2;
    Label2=tmp;
end
...
for i=1:nClass1
	for j=1:nClass2
        if(numel(L1 == Label1(i)) ~= numel(L2 == Label2(j)))
            disp()
        end
...
function p=purity(classLabels, clusterLabels)
    numOfSample=length(classLabels);
    numOfClass=max([unique(classLabels),unique(clusterLabels)]);
%     numOfCluster=length(unique(clusterLabels));
    
    % 矩阵，行是原本类的indicator，列是新cluster的indicator，相乘，按行max，sum，除以总和 
    ClassIndicator=zeros(numOfSample,numOfClass);
    ClassIndicator([1:numOfSample]+(classLabels-1)*numOfSample)=1;
    ClusterIndicator=zeros(numOfSample,numOfClass);
    ClusterIndicator([1:numOfSample]+(clusterLabels-1)*numOfSample)=1;
    
    p=sum(max(ClassIndicator'*ClusterIndicator))/numOfSample;    
end
